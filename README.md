...
# Fiction Bank — LCR Regulatory Model

**This repository represents Fiction Bank Finance's LCR calculation logic**, forked from [finos/open-reg-tech-us-lcr](https://github.com/finos/open-reg-tech-us-lcr).  It is one of three repos in the Fiction Bank demo sandbox for DataHub:

| Repo | Team | Purpose |
|------|------|---------|
| `open-reg-tech-us-lcr` (this repo) | Regulatory / Finance | LCR reference implementation (Morphir/Elm); forked from FINOS |
| [`fiction-bank-finance`](../fiction-bank-finance) | Finance | Reference data, domain ontology, business query corpus |
| [`sec-lending-fiction-bank`](../sec-lending-fiction-bank) | Securities Lending | US + UK position model, rate normalization, cross-region harmonization |

The Morphir/Elm model encodes the Basel III Liquidity Coverage Ratio calculation rule as executable functional code.  The compiled IR (`morphir-ir.json`) drives:

- **DataHub ingestion**: `morphir_to_datahub.py` emits 13 FR 2052a logical model datasets with field-level glossary term mappings to `urn:li:glossaryTerm:regulation.lcr.*`
- **SKOS taxonomy**: `src/Regulation/US/taxonomy.ttl` (generated by `morphir-elm make -t semantic`)
- **LCR ontology stub**: `ontology/lcr-terms.ttl` — human-authored SKOS concepts for higher-level LCR regulatory concepts, following the same naming convention as `finance-terms.ttl` and `sec-lending-terms.ttl`

## Domain assignment

LCR calculation outputs belong to the **Finance > Regulatory Reporting** domain in DataHub:

```
urn:li:domain:fiction-bank.finance.regulatory-reporting
```

LCR-specific glossary terms from `ontology/lcr-terms.ttl` are organized under the `RegulatoryReporting` term group, separate from (but crosslinked to) the `Finance` term group in `fiction-bank-finance`.

**Prerequisite**: Run `fiction-bank-firm-data-office/src/setup_domains.py` before running `morphir_to_datahub.py`.
